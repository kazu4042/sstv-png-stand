...
def process_upload(filepath, original_filename, job_id, app, user_id):
    ...
    try:
        ...
        # ===== Step1: 指定されたモードのみでデコードを実行 =====
        update_job(job_id, progress=6, status=f"音声信号の高速デコード中 ({mode_name} モード)...")
        ...
        if not decoded_bits_list:
            update_job(
                job_id, progress=100, status="エラー",
                error=f"SSTV Turbo ({mode_name} モード) の信号が検出できませんでした。音声をスピーカーから再生し、スマホのマイクを近づけて最初（1000Hzのピ音）から最後までしっかり録音したファイルを選択してください。"
            )
            return
