...
def setup_demo_audio():
    ...
    # 1. JPEG 音声の確認・セットアップ
    if not os.path.exists(jpeg_target) or os.path.getsize(jpeg_target) < 1000000:
        # compare_jpeg.wav があればコピー
        source_jpeg = os.path.join(ROOT_DIR, "data", "digital_turbo_jpeg", "audio", "compare_jpeg.wav")
        if os.path.exists(source_jpeg) and os.path.getsize(source_jpeg) > 10000000:
            print(f"📦 既存の高品質 JPEG 音声をコピー中: {source_jpeg} -> {jpeg_target}")
            shutil.copyfile(source_jpeg, jpeg_target)
        else:
            print("⚙️ 長崎人工衛星画像 (test.jpg) から JPEG 音声をエンコード生成中 (約30秒)...")
        ...
        print(f"✅ JPEG デモ音声は既に配置済みです: {jpeg_target} ({os.path.getsize(jpeg_target)} bytes)")

    # 2. PNG 音声の確認・セットアップ
    if not os.path.exists(png_target) or os.path.getsize(png_target) < 1000000:
