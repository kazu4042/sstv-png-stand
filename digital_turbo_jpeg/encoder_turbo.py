    ...
    def encode(self, image_path, output_wav_path=None):
        ...
        if not os.path.exists(image_path):
            ...
            if os.path.exists(alt_path):
                image_path = alt_path
            elif os.path.exists(os.path.join(root_dir, "digital_jpeg", "test.jpg")):
                image_path = os.path.join(root_dir, "digital_jpeg", "test.jpg")
            else:
        ...
        print(f"[Encode] Starting TURBO JPEG: {config.WIDTH}x{config.HEIGHT} (Tile: {config.TILE_SIZE}x{config.TILE_SIZE} => {config.TILE_COUNT_X}x{config.TILE_COUNT_Y} tiles)")
        ...
        for ty in range(config.TILE_COUNT_Y):
            for tx in range(config.TILE_COUNT_X):
                ...
                with io.BytesIO() as bio:
                    tile_img.save(bio, format="JPEG", quality=config.JPEG_QUALITY)
                ...
                if payload_length > max_len:
                    raise ValueError(f"Tile ({tx},{ty}) JPEG data ({payload_length} bytes) exceeds limit ({max_len} bytes).")
                ...
                payload_bits = self.bytearray_to_bitstream(jpeg_data)
