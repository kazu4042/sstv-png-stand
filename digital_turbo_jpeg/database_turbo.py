...
class PacketDatabaseTurboJPEG:
    """SSTV Turbo JPEG 用パケットデータベース (SQLite)"""
    def __init__(self, log_dir):
        os.makedirs(log_dir, exist_ok=True)
        self.db_path = os.path.join(log_dir, "sstv_packets_turbo_jpeg.db")
    ...
    def _init_db(self):
        with self.conn:
            ...
            try:
                ...
                pass  # Already exists
            self.conn.execute("CREATE INDEX IF NOT EXISTS idx_turbo_jpeg_packets_image_tile ON packets (image_id, tile_x, tile_y, payload_length)")
            self.conn.execute("CREATE INDEX IF NOT EXISTS idx_turbo_jpeg_packets_image_id ON packets (image_id)")
            self.conn.execute("CREATE INDEX IF NOT EXISTS idx_turbo_jpeg_packets_user_id ON packets (user_id)")
            self.conn.execute("CREATE INDEX IF NOT EXISTS idx_turbo_jpeg_packets_image_user ON packets (image_id, user_id)")
            self.conn.execute("CREATE INDEX IF NOT EXISTS idx_turbo_jpeg_packets_tile_group ON packets (image_id, tile_y, tile_x)")
    ...
    def get_snr_analytics(self) -> dict[str, Any]:
        ...
        try:
            ...
            print(f"[PacketDB-JPEG] get_snr_analytics error: {e}")
            return {
                "total_packets": 0,
                "avg_snr": 0.0,
                "max_snr": 0.0,
    ...
    def get_hourly_packet_traffic(self, period='today'):
        ...
        try:
            ...
            print(f"[PacketDB-JPEG] get_hourly_packet_traffic error: {e}")
    ...
    def optimize_database(self):
        ...
        try:
            ...
            print(f"[PacketDB-JPEG] Optimize error: {e}")
            return False
...
# 互換エイリアス
PacketDatabaseTurbo = PacketDatabaseTurboJPEG
